  ...
  return (
    <div className="relative pt-28 pb-20 px-6">
      <div className="max-w-[1200px] mx-auto">
        <nav className="flex items-center gap-2 text-sm text-[#80504A] mb-8">
          ...
          <ChevronRight className="w-3.5 h-3.5" />
          <span className="text-[#BE8A83]">Best {category} 2026</span>
        </nav>

        <header className="mb-12">
          <div className="flex items-center gap-4 mb-4">
            <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-[#E63946]/20 to-[#FF6B6B]/20 flex items-center justify-center">
              <Sparkles className="w-7 h-7 text-[#FF6B6B]" />
            </div>
            <div>
              <h1 className="text-4xl md:text-5xl font-extrabold text-[#FEF2F0] tracking-tight">
                Best {category} Software in 2026
              </h1>
              <p className="text-base text-[#BE8A83] mt-1">
          ...
          {topTool && (
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-[#401616] border border-[#F59E0B]/30 rounded-full text-sm text-[#F59E0B]">
              <Star className="w-4 h-4 fill-[#F59E0B]" />
              Top Pick:{" "}
              <Link
                href={`/tools/${topTool.id}`}
                className="font-bold hover:text-[#FEF2F0] transition-colors underline underline-offset-2"
              >
                {topTool.name}
              </Link>
              <span className="text-[#80504A]">— {topTool.rating}/5</span>
            </div>
          )}
        </header>
        ...
        <section className="mb-14">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b border-[#5F1E1E]">
                  <th className="text-left py-3 px-4 text-[#BE8A83] font-semibold">
                    Tool
                  </th>
                  <th className="text-left py-3 px-4 text-[#BE8A83] font-semibold">
                    Rating
                  </th>
                  <th className="text-left py-3 px-4 text-[#BE8A83] font-semibold hidden md:table-cell">
                    Pricing
                  </th>
                  <th className="text-left py-3 px-4 text-[#BE8A83] font-semibold hidden lg:table-cell">
                    Reviews
                  </th>
                  <th className="text-right py-3 px-4 text-[#BE8A83] font-semibold">
                    Details
                  </th>
                </tr>
              </thead>
              <tbody>
                {tools.slice(0, 10).map((tool, idx) => {
                  ...
                  return (
                    <tr
                      key={tool.id}
                      className="border-b border-[#5F1E1E]/60 hover:bg-[#2D0F0F]/80 transition-colors"
                    >
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-lg bg-[#401616] flex items-center justify-center shrink-0">
                            <Icon className="w-4 h-4 text-[#E63946]" />
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="font-bold text-[#FEF2F0] whitespace-nowrap">
                                {tool.name}
                              </span>
                              {idx === 0 && (
                      ...
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="font-bold text-[#FEF2F0]">
                            {tool.rating}
                          </span>
                        </div>
                      </td>
                      <td className="py-4 px-4 text-[#BE8A83] hidden md:table-cell">
                        <div className="flex items-center gap-1">
                          <DollarSign className="w-3.5 h-3.5 text-[#FF6B6B]" />
                          <span>{tool.pricing}</span>
                        </div>
                      </td>
                      <td className="py-4 px-4 text-[#80504A] hidden lg:table-cell">
                        {tool.reviewCount.toLocaleString()}
                      </td>
                      <td className="py-4 px-4 text-right">
                        <Link
                          href={`/tools/${tool.id}`}
                          className="inline-flex items-center gap-1 text-[#E63946] hover:text-[#FF6B6B] transition-colors text-xs font-semibold"
                        >
        ...
        <section className="mb-14">
          <h2 className="text-2xl font-bold text-[#FEF2F0] mb-6">
            Detailed Reviews
          </h2>
            ...
            {tools.slice(0, 6).map((tool, idx) => {
              ...
              return (
                <Link
                  href={`/tools/${tool.id}`}
                  key={tool.id}
                  className="group bg-[#2D0F0F] border border-[#5F1E1E] rounded-xl p-6 card-hover"
                >
                  <div className="flex items-start gap-4 mb-4">
                    <div className="relative">
                      <div className="w-12 h-12 rounded-lg bg-[#401616] flex items-center justify-center group-hover:scale-110 transition-transform">
                    ...
                    <div className="min-w-0 flex-1">
                      <h3 className="font-bold text-[#FEF2F0] group-hover:text-[#E63946] transition-colors truncate">
                        {tool.name}
                      </h3>
                      <div className="flex items-center gap-1 mt-1">
                        <Star className="w-3.5 h-3.5 text-[#F59E0B] fill-[#F59E0B]" />
                        <span className="text-xs font-bold text-[#FEF2F0]">
                          {tool.rating}
                        </span>
                        <span className="text-xs text-[#80504A]">
                          ({tool.reviewCount.toLocaleString()})
                        </span>
                      </div>
                    </div>
                  </div>
                  <p className="text-sm text-[#BE8A83] leading-relaxed line-clamp-2 mb-3">
                    {tool.description}
                  </p>
                  <div className="text-xs text-[#FF6B6B] font-medium mb-3">
                    {tool.pricing}
                  </div>
                  <div className="flex items-center justify-between pt-3 border-t border-[#5F1E1E]">
                    <span className="text-xs text-[#80504A] capitalize">
        ...
        <section className="mb-14">
          <div className="bg-[#2D0F0F] border border-[#5F1E1E] rounded-2xl p-8">
            <h2 className="text-2xl font-bold text-[#FEF2F0] mb-4 flex items-center gap-2">
              <ArrowRight className="w-6 h-6 text-[#E63946]" />
              {category} Software Buying Guide
            </h2>
            <p className="text-[#BE8A83] leading-relaxed text-base">
              {buyingGuide}
            </p>
          </div>
        </section>
        ...
        <section className="mb-14">
          ...
          <div className="space-y-4">
            {faqs.map((faq, i) => (
              <details
                key={i}
                className="bg-[#2D0F0F] border border-[#5F1E1E] rounded-xl group"
              >
                <summary className="px-6 py-4 cursor-pointer text-[#FEF2F0] font-semibold text-sm flex items-center justify-between group-open:text-[#FF6B6B] transition-colors">
                  {faq.q}
                  <ChevronRight className="w-4 h-4 text-[#80504A] group-open:rotate-90 transition-transform shrink-0 ml-4" />
                </summary>
                <div className="px-6 pb-4 text-sm text-[#BE8A83] leading-relaxed">
                  {faq.a}
                </div>
              </details>
            ))}
        ...
        {/* CTA */}
        <section className="bg-gradient-to-r from-[#401616] to-[#2D0F0F] border border-[#5F1E1E] rounded-2xl p-8 md:p-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h2 className="text-xl font-bold text-[#FEF2F0] mb-2">
          ...
          <div className="flex flex-wrap gap-3">
            <Link
              href={`/category/${params.slug}`}
              className="px-6 py-3 bg-[#2D0F0F] border border-[#5F1E1E] text-[#FEF2F0] text-sm font-medium rounded-full hover:border-[#802828] transition-colors"
            >
            ...
            {topTool && (
              <Link
                href={`/tools/${topTool.id}`}
                className="px-6 py-3 bg-[#E63946] hover:bg-[#C53030] text-white text-sm font-medium rounded-full transition-colors"
              >
