    ...
    <div className="relative pt-28 pb-20 px-6">
      <div className="max-w-[1200px] mx-auto">
        ...
        <nav className="flex items-center gap-2 text-sm text-[#80504A] mb-8">
          ...
          <span className="text-[#BE8A83]">{category} Software</span>
        </nav>

        {/* Header */}
        <header className="mb-10">
          <div className="flex items-center gap-3 mb-4">
            <div className="w-12 h-12 rounded-xl bg-[#401616] flex items-center justify-center">
          ...
          <p className="text-[#BE8A83] leading-relaxed max-w-3xl text-base">
            {description}
          </p>
        </header>
        ...
        <div className="flex flex-wrap gap-3 mb-10">
          <Link
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#E63946] hover:bg-[#C53030] text-white text-sm font-medium rounded-full transition-colors"
          >
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-4 mb-4">
                    <div className="w-12 h-12 rounded-lg bg-[#401616] flex items-center justify-center group-hover:scale-110 transition-transform shrink-0">
                  ...
                  <p className="text-sm text-[#BE8A83] leading-relaxed line-clamp-2 mb-4">
                    {tool.description}
                  </p>
                  <div className="flex items-center justify-between pt-4 border-t border-[#5F1E1E]">
                    <span className="text-xs text-[#80504A]">{tool.pricing}</span>
                    <span className="text-sm text-[#E63946] font-semibold group-hover:text-[#FF6B6B] transition-colors flex items-center">
        ...
        {relatedPosts.length > 0 && (
          <section className="mt-16">
            <div className="flex items-center gap-2 mb-6">
              ...
              <h2 className="text-xl font-bold text-[#FEF2F0]">
                Related Guides
              </h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {relatedPosts.map((post) => (
                <Link
                  key={post.slug}
                  href={`/blog/${post.slug}`}
                  className="group bg-[#2D0F0F] border border-[#5F1E1E] rounded-xl p-5 card-hover"
                >
                  <span className="inline-block text-[10px] font-semibold uppercase tracking-wider text-[#E63946] bg-[#401616] px-2 py-0.5 rounded mb-3">
                    {post.category}
                  </span>
                  <h3 className="font-bold text-[#FEF2F0] group-hover:text-[#E63946] transition-colors leading-snug line-clamp-2 mb-2">
                    {post.title}
                  </h3>
                  <p className="text-xs text-[#BE8A83] line-clamp-2">
                    {post.excerpt}
                  </p>
                </Link>
              ))}
