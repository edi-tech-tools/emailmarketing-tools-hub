#!/usr/bin/env python3
"""Generate and append blog post to blog-posts.ts"""
import re

# Read existing blog-posts.ts
with open('/home/edi/emailmarketing-tools-hub/app/data/blog-posts.ts', 'r') as f:
    content = f.read()

# The blog content - NO backticks used, all replaced with single quotes
# NO ${} patterns exist in this content
blog_content = '''# Email Automation Workflows: 7 High-Impact Sequences You Can Deploy This Week
...
# Verify NO backticks, NO ${} in the content
assert '`' not in blog_content, 'Backticks found in content!'
assert '${' not in blog_content, 'Dollar-brace found in content!'
...
if new_content.endswith('];'):
    new_content = new_content[:-2]  # Remove ];
    new_content += '\n' + blog_entry
else:
    print('ERROR: File does not end with ];')
    exit(1)

with open('/home/edi/emailmarketing-tools-hub/app/data/blog-posts.ts', 'w') as f:
    f.write(new_content)
...
with open('/home/edi/emailmarketing-tools-hub/app/data/blog-posts.ts', 'r') as f:
    verify = f.read()
print('New slug count:', verify.count('slug:'))
print('File ends with:', verify[-20:])
